...
public class AESCipher implements Cipher {
    ...
    @Override
    public byte[] apply(OpMode opMode, byte[] message, byte[] publicKey, byte[] privateKey) { // for symmetric ciphers publicKey is used.
        var key = publicKey;

        if (key.length != keyBitLength) {
            throw new IllegalArgumentException("Invalid key bit length");
        }

        if (opMode == OpMode.ENCRYPT) {
            int numRounds = 10 + (((keyBitLength * 4 - 128) / 32));
            int[][] state;
            int[][] keymatrix = keySchedule(ByteBuffer.wrap(publicKey).toString());

            state = new int[4][4];
            for (int i = 0; i < 4; i++) {
                for (int j = 0; j < 4; j++) {
                    var messageString = ByteBuffer.wrap(message).toString();
                    state[j][i] = Integer.parseInt(messageString.substring((8 * i) + (2 * j), (8 * i) + (2 * j + 2)), 16);
                }
            }
            ...
            return MatrixToString(state).getBytes();
        } else {
